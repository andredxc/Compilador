...
void semanticCheckDeclarations(AST_NODE* node){
    ...
    if(node){
        ...
        if(node->type == AST_DEC_VAR_GLOB){
            ...
            if(node->son[0]->type == AST_DEC_VAR_SHORT && (node->son[0]->symbol->symbol.type == LIT_INTEGER || node->son[0]->symbol->symbol.type == LIT_CHAR)){
                node->symbol->symbol.dataType = DATATYPE_SHORT;
                node->symbol->symbol.nature = NATURE_VARIABLE;
            }
            else if(node->son[0]->type == AST_DEC_VAR_LONG && (node->son[0]->symbol->symbol.type == LIT_INTEGER || node->son[0]->symbol->symbol.type == LIT_CHAR)){
                node->symbol->symbol.dataType = DATATYPE_LONG;
                node->symbol->symbol.nature = NATURE_VARIABLE;
            }
            else if(node->son[0]->type == AST_DEC_VAR_BYTE && (node->son[0]->symbol->symbol.type == LIT_INTEGER || node->son[0]->symbol->symbol.type == LIT_CHAR)){
                node->symbol->symbol.dataType = DATATYPE_BYTE;
                node->symbol->symbol.nature = NATURE_VARIABLE;
            }
            else if(node->son[0]->type == AST_DEC_VAR_FLOAT && (node->son[0]->symbol->symbol.type == LIT_REAL)){
                node->symbol->symbol.dataType = DATATYPE_FLOAT;
                node->symbol->symbol.nature = NATURE_VARIABLE;
            }
            else if(node->son[0]->type == AST_DEC_VAR_DOUBLE && (node->son[0]->symbol->symbol.type == LIT_REAL)){
            ...
            //Declaração de vetor global
                ...
                //Verifica a quantidade e o dataType dos valores de inicialização
                if(node->son[0]->symbol->symbol.type == LIT_INTEGER){
                    ...
                    if(arraySize == 0){
                        ...
                        for(i = 0; i < arraySize; i++){
                            ...
                            fprintf(stderr, "Verificando vetor, %d de %d\n", i+1, arraySize);
                            ...
                            if((node->son[0]->type == AST_DEC_VEC_BYTE || node->son[0]->type == AST_DEC_VEC_SHORT || node->son[0]->type == AST_DEC_VEC_LONG) &&
                                (astBuffer->symbol->symbol.type != LIT_INTEGER && astBuffer->symbol->symbol.type != LIT_CHAR)){
                                //Tipo incorreto na declaração de vetor inteiro
                                fprintf(stderr, "ERRO, vetor [%s] declarado com valor real\n", node->symbol->symbol.text);
                                _errorStatus = 1;
                            }
                            else if((node->son[0]->type == AST_DEC_VEC_FLOAT || node->son[0]->type == AST_DEC_VEC_DOUBLE) &&
                                (astBuffer->symbol->symbol.type != LIT_REAL)){
                                fprintf(stderr, "ERRO, vetor [%s] declarado com valor inteiro\n", node->symbol->symbol.text);
                                _errorStatus = 1;
                            }
                            else{
                ...
                //Determina o dataType e o nature na tabela hash
                node->symbol->symbol.nature = NATURE_ARRAY;
                switch(node->son[0]->type){
                    case AST_DEC_VEC_BYTE: node->symbol->symbol.dataType = DATATYPE_BYTE; break;
                    case AST_DEC_VEC_SHORT: node->symbol->symbol.dataType = DATATYPE_SHORT; break;
                    case AST_DEC_VEC_LONG: node->symbol->symbol.dataType = DATATYPE_LONG; break;
                    case AST_DEC_VEC_FLOAT: node->symbol->symbol.dataType = DATATYPE_FLOAT; break;
                    case AST_DEC_VEC_DOUBLE: node->symbol->symbol.dataType = DATATYPE_DOUBLE; break;
                    default: fprintf(stderr, "ERRO interno, tipo não reconhecido para a variável [%s]\n", node->symbol->symbol.text);
                }
            }
            else{
